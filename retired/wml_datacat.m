...
for s = 1:3
    ...
    grp_contents = dir(fullfile(rootDir, 'wml-data', 'wml-data-mri', blprojectid));
    ...
    for i = 1:size(grp_contents, 1)
        ...
        % Get contents of the directory where the tract measures for this subject are stored.
        sub_contents_tractprofiles = dir(fullfile(grp_contents(i).folder, grp_contents(i).name,  '/dt-neuro-tractmeasures*/*.csv'));
        
        % Read in data for this subject and this tract.
        data_temp = readtable([sub_contents_tractprofiles.folder filesep sub_contents_tractprofiles.name]);
        
        % Clip beginning and end to avoid partial volume effects.
        data_temp = data_temp(21:end-20, :);
        
        % Append session indicator.
        data_temp.session = repmat(s, [size(data_temp, 1) 1]);
...
%
% Save all variables.
save(fullfile(rootDir, 'supportFiles', 'wml_data_mri_longform.mat'), 'data_tbl')
%
% Write out table.
writetable(data_tbl, fullfile(rootDir, 'supportFiles', 'wml_data_mri_longform.csv'));
...
%                 tractID = list_tract{t};
                ...
                idx_here = find(data.session == ses & ...
                    strcmp(data.subjectID, list_subs{sub}) & ...
        ...
        data_out = cat(2, list_subs, data_out);
...
%         
        % Save all variables.
        save(fullfile(rootDir, 'supportFiles', ['wml_data_session' num2str(ses) '_' wm_measure '.mat']), 'data_tbl')
%         
        % Write out table.
        writetable(data_out, fullfile(rootDir, 'supportFiles', ['wml_data_session' num2str(ses) '_' wm_measure '.csv']));
