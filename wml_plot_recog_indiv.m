...
% Important to replace with NaN and not remove the trial entirely so that
% the number of trials is the same across participants.
tpidx = find(recog.truepositive == 0);
recog.RT(tpidx) = NaN;
...
%% Fit a double exponential to each person's Recog data to get learning rate.
...
% didx = find(recog.day == 1);
% recog = recog(didx, :);

% Don't include day 5, for now.
didx = find(recog.day ~= 5);
recog = recog(didx, :);
...
for s = 1:length(subjectlist)
    ...
    disp([num2str(subjectlist(s)) ', ' num2str(length(sidx))])
