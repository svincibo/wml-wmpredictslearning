...
options.StartPoint = [0.3488 -0.04199 2.936 -0.000278];
options.Lower = [0.5*0.3488 2*-0.04199 0.5*2.936 2*-0.000278]; %.5*options.StartPoint;
options.Upper = [2*0.3488 0.5*-0.04199 2*2.936 0.5*-0.000278]; %2*options.StartPoint;
options.Robust = 'on';
...
for s = 1:length(subjectlist)
    ...
    % Fit.
    [f, f2] = fit(x, y, fittype, options); %, 'Robust', 'Lar');
